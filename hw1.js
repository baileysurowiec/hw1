 ...
 * *****************
 * 
 * Implement the Circle class below. It needs to have a radius(Number) property and a color(String) 
 * properties. Both properties should be set in a constructor
 * Additionally, the Circle class should have a method called `calcArea` which will return the area 
 * (radius*radius*pi) of the circle. 
...
const Product = class Product {
    constructor(singleStringInput){
        let stringArray = singleStringInput.split(",");
        this.name = stringArray[0];
        this.price = Number(stringArray[1]);
        this.availability = stringArray[2];
    }
    ...
    static inStock = (products) => {
        return products.filter(stock => stock.availability == "In Stock" )
    }
    ...
    static halfOff = (products) => {
        let productArr =  products.map(newProduct => new Product( `${newProduct.name}, ${newProduct.price *0.5}, ${newProduct.availability}`));
        return productArr;
    }
    ...
    static printProducts = (products) => {
        products.forEach( product => console.log(
            `Product: ${product.name}, Cost: ${new Intl.NumberFormat
                ('us-US', {style: 'currency', currency:'USD'}).format(product.price)}, Availability: ${product.availability == 'In Stock' ? 'Yes' : 'No'}`
        ));
    }
 };
